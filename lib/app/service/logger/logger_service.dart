...
import 'package:clublab_lite/app/service/logger/printer/app_log_printer.dart';
...
class LoggerService {
  ...
  static Future<void> init({
    ...
    _logger
        ?.i('LoggerService.init(): firebaseInitResult = $firebaseInitResult');
  ...
  static Future<LogOutput> _getLogOutput() async {
    if (_saveToFile) {
      if (_logFilePath == null) {
        ...
        debugPrint(
            '!!! LoggerService._getLogOutput(): logFilePath = $_logFilePath');
      }
  ...
  static Future<void> _sendLogsToRemoteStorage({
    ...
    final forSend =
        await logDir.list().where((e) => e.isLogFile(forShare: true)).toList();
